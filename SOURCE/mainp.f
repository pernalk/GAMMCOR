...
C     NBasis READ FROM SIRIUS.RST
      If(IDALTON.Eq.1) Call basinfo(NAO,NBasis,'SIRIUS.RST','DALTON')
      If(ITREXIO.Eq.1) Call basinfo(NAO,NBasis,TrexFile,'TREXIO')
CC
C     *************************************************************************
C
...
C     CALCULATE THE DIMENSIONS
      If(IMOLPRO.Eq.1) Then
C        Call CheckNBa(NBasis,Title)
        Call basinfo(NAO,NBasis,'AOONEINT.mol','MOLPRO')
      EndIf
C
...
C
      ElseIf(ITREXIO.Eq.1) Then
C
      Call ReadTREXIO(XKin,XNuc,ENuc,Occ,URe,TwoEl,UMOAO,
     $                NAO,NBasis,NInte1,NInte2,TrexFile)
C
      EndIf
C
      If(IFunSR.Eq.5) Then
