...
contains
      ...
      subroutine attr_d(fid, path, name, val)
            ...
            if (ierr /= 0) then
                  print *, "h5_attr: missing ", trim(name), " on ", trim(path)
                  error stop
            end if
            val = buf(1)
      end subroutine attr_d
      ...
      subroutine attr_s(fid, path, name, val)
            ...
            if (vlen) then
                  ...
                  if (ierr /= 0) then
                        print *, "h5_attr: read failed ", trim(name), " on ", trim(path)
                        error stop
                  end if
