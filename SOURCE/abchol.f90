...
! use in-memory cholesky (FF|K) vectors if available, otherwise read from disk
...
! use in-memory cholesky if available, otherwise read from disk
if (NCholesky_stored > 0) then
   NCholesky = NCholesky_stored
   MatFF => CholVecsFF
else
