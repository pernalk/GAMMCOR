#define SAPT_INTERFACE_DEBUG 11
...
! read coefficient, occupancies
 ...
 ! for dSRS/TREXIO, set occupation numbers later
 ! based on IRef1 declared in input
...
! Cholesky decomposition
 ...
 call save_CAONO(Ca,SAPT%monA%CMO,SAPT%monA,Flags%IdSRS,NAO,NBasis)
 call save_CAONO(Cb,SAPT%monB%CMO,SAPT%monB,Flags%IdSRS,NAO,NBasis)
...
! read & transform 2-rdm / trexio
 if(SAPT%InterfaceType==5) then
   ...
   if (Flags%IdSRS/=1) call rw_trexio_rdm2(SAPT%monA,AuxA,NBasis)
   if (Flags%IdSRS/=1) call rw_trexio_rdm2(SAPT%monB,AuxB,NBasis)
   if (Flags%ICASSCF==1) call rw_trexio_save_rdm2file(Flags,SAPT%monA)
   if (Flags%ICASSCF==1) call rw_trexio_save_rdm2file(Flags,SAPT%monB)
 endif
...
if (rc /= TREXIO_SUCCESS) then
  stop 'No nuclar repulson energy in file'
end if
rc = trexio_read_nucleus_repulsion(f,repulsion)
Mon%PotNuc = repulsion
...
elseif(Flags%ICASSCF==1) then
   ...
   itmp = 0
   tmp = 0d0
   ! Quantum Package holds the entitre 1-rdm (not only the active part)
   !
   ! the 1-^-10 threshold is also used for Molpro
   !
   do i=1,NBasis
      tmp = tmp + EVal(i) / 2d0
      if(EVal(i)>1d-10) itmp = itmp + 1
      if(abs(EVal(i)-2d0)<1d-10) Mon%INAct = Mon%INAct + 1
   enddo
...
elseif (Flags%ICASSCF == 1 ) then
   Mon%Occ = 0
   Mon%Occ(1:NOccup) = EVal(1:NOccup) / 2
   ! inactive part has to be set to 1d0 for FRDM2 function to work!
   do i=1,Mon%INAct
      Mon%Occ(i) = 1d0
   enddo
endif
...
! b) read all 1-trdms, and store in MO in mon%Trdm1(nstates*(nstates-1)/2,NBasis,NBasis)
...
!
! IMPORTANT! 
! Replace previous, state-averaged Occ with 1-RDM(IRef1)! 
!
implicit none
...
!                   Mon%trdm1(itr,:,:),NBasis)
      ...
      print*, '1-TRDM MO, RefState = ',RefState
      do j=1,NOccup
...
subroutine read_1rdm_1trdm_trexio(NAO,NBasis,mon)
!
! this is nearly identical to molpro version
! except for read-trexio interface
! can be merged into a single procedure
!
! a) read all 1-rdms,  transform to NO of RefState, and store in mon%rdm1(nstates)
...
#if SAPT_INTERFACE_DEBUG > 5
print*, 'NOccup in read_1rdm_1trdm    : ', NOccup
print*, 'Inact  in read_1rdm_1trdm    : ', Mon%INAct
print*, 'NAct   in read_1rdm_1trdm    : ', Mon%NAct
#endif

write(lout,'(/1x,"Monomer",a2)') prefix
write(lout,'(1x,a,i3)') 'The number of states from SA-CAS  :',  nstates
write(lout,'(1x,a,i3,/)') 'Reference state for dSRS          :',iref
...
do ist=1,nstates
   ...
   !call read_txt_1mat_trexio(filenames(ist),work,NOccup,NOccup)
   !!! MAYBE TRY USING PREPARE_NO_MOLPRO here ??? call sq_to_triang(work1d,OneRdm,NBasis)
   !! reduce to act only
   !do j=1,Mon%NAct
   !   do i=1,Mon%NAct
   !      AuxRDM(ist,i,j) = work(Mon%INAct+i,Mon%INAct+j)
   !   enddo
   !enddo

   ! what about the virt-virt block?
   !call Diag8(AuxRDM(ist,:,:),Mon%NAct,Mon%NAct,EVal(ist,1:Mon%NAct),workTr)
   ...
   ! obtain AO-->NO transformation matrix
   Mon%CAONO(ist,1:NAO,1:NBasis) = CAOMO(1:NAO,1:NBasis)
   call prepare_no_trexio(CMONO(ist,:,:),Mon%CAONO(ist,:,:),NAO,NBasis)
...
! Replace Occ and CICoef
! Occ and CICoef correspond to Mon%IRef1 state!
!Print*, 'Warning! Using Occupation numbers for State IRef1 =',Mon%IRef1
Mon%Occ(1:NBasis) = Mon%rdm1(Mon%IRef1,1:NBasis)
do i=1,NBasis
   Mon%CICoef(i) = sign(sqrt(Mon%Occ(i)),Mon%Occ(i)-0.5d0)
enddo

! read all 1-TRDMs in MO
allocate(Mon%trdm1(nnstates,NBasis,NBasis))
...
end subroutine read_1rdm_1trdm_trexio

subroutine read_txt_1mat_trexio(filename,Mat,dim1,dim2)
!
! tmp subroutine to read 1-el matrices
! dumped from QP2
! used only by read_1rdm_1trdm_trexio
!
implicit none

character(*) :: filename
character(100) :: header_line
integer,intent(in) :: dim1,dim2
double precision,intent(out) :: Mat(dim1,dim2)
...
double precision :: val
  ...
  read(10, '(A)', iostat=i) header_line
    if (i /= 0) then
      print *, "Error reading header in file:", filename
      close(10)
      return
  end if

  do
      read(10, *, iostat=i) k, l, val
      if (i /= 0) exit  ! Exit when end of file or error is encountered
      Mat(k, l) = val
  end do
  close(10)

end subroutine read_txt_1mat_trexio

subroutine read_txt_2mat_trexio(filename,Mat,n)
!
! tmp subroutine to read 2-el matrices
! dumped from QP2
!
implicit none
...
character(100) :: header_line
integer,intent(in) :: n
double precision,intent(out) :: Mat(n,n,n,n)
...
do
    read(10, *, iostat=io) i, j, k, l, val
    if (io /= 0) exit
    Mat(i, j, k, l) = val
end do
close(10)

end subroutine read_txt_2mat_trexio

subroutine read_2rdm_2trdm_molpro(NBasis,mon)
!
! read 2-RDMs for all states, transform them from MO-->NO
! using Mon%CMONO
!
implicit none
...
do ist=1,nstates
   ...
   block
   double precision :: xtrace
   xtrace=0d0
   do k=1,NOccup
      do i=1,NOccup
      xtrace = xtrace + mon%rdm24(ist,i,i,k,k)
      enddo
   enddo
   print*, 'xtrace =',ist,xtrace
   end block
...
subroutine read_2rdm_2trdm_trexio_tmp(NAO,NBasis,mon)
!
! a) read active 2-RDMs for all states,
!    transform them from MO-->NO, using Mon%CMONO
!    expand to NOccup = Inact+Act
! b) read 2-TRDMs
!
! c) save 2-RDM for IRef1 in NO rep in Mon%RDM2val
!
implicit none
...
do ist=1,nstates
  call read_txt_2mat_trexio(filenames(ist),rdm24act,Mon%NAct)
  ! CMONO --> CMONOAct
  work = 0d0
  do j=1,Mon%NAct
     do i=1,Mon%NAct
        work(i,j) = Mon%CMONO(ist,Mon%INAct+i,Mon%INAct+j)
        !print*,i,j,work(i,j)
     enddo
  enddo
  ...
  do k=1,Mon%NAct
     do i=1,Mon%NAct
        xtrace = xtrace + 0.5d0*rdm24act(i,k,i,k)
     enddo
  enddo
  rdm24act=0.5d0*rdm24act
  write(lout, '("2-RDM (act) no. ",I0," has trace = ",F12.6)') ist,xtrace
  ! expand 2-rdm from act to noccup 
  do l=1,NOccup
     do k=1,NOccup
        do j=1,NOccup
           do i=1,NOccup
              mon%rdm24(ist,i,j,k,l) = FRDM24(i,k,j,l,rdm24act,&
                                       mon%rdm1(ist,:),Ind2,Mon%NAct,NBasis)
              !write(lout,'(a,4i2,2f12.6)') 'ver1',i,j,k,l,rdm24act(i,k,j,l),mon%rdm24(ist,i,j,k,l)
           enddo
        enddo
     enddo
  enddo
...
! read active 1trdm
if(Mon%Monomer==1) then
 filenames(1) = 'A_2trdm_2_1.txt'
 filenames(2) = 'A_2trdm_3_1.txt'
elseif(Mon%Monomer==2) then
 filenames(1) = 'B_2trdm_2_1.txt'
 filenames(2) = 'B_2trdm_3_1.txt'
endif

! check how they are constructed from ERPA vecs
call read_txt_2mat_trexio(filenames(1),rdm24act,Mon%NAct)
...
!      idx_m = idx_buf(3,i)
!      idx_n = idx_buf(4,i)
!      if (idx_k <= Mon%NAct .and. idx_l <= Mon%NAct .and. idx_m <= Mon%NAct .and. idx_n <= Mon%NAct) then
!         write(6,'(4i3,f14.10)') idx_k,idx_l,idx_m,idx_n,val_buf(i)
!         rdm24act(idx_k,idx_l,idx_m,idx_n) = val_buf(i)
!         !if(idx_k.eq.idx_m.and.idx_l.eq.idx_n) then
!         if(idx_k.eq.idx_l.and.idx_m.eq.idx_n) then
!            xtrace= xtrace + 0.5d0*val_buf(i)
!         endif
!      endif
!   enddo
...
!
!print*, 'xtrace 2-trdm trexio =',xtrace

! CMONO --> CMONOAct
work = 0d0
do j=1,Mon%NAct
   do i=1,Mon%NAct
      work(i,j) = Mon%CMONO(Mon%IRef1,Mon%INAct+i,Mon%INAct+j)
   enddo
enddo
write(lout,'(1x,"Norm of 2-TRDM MO", F12.6)') norm2(rdm24act)

call TrRDM24_dgemm(rdm24act,work(1:Mon%NAct,1:Mon%NAct),Mon%NAct)
print*, '2-TRDM NO norm', norm2(rdm24act)
...
do k=1,Mon%NAct
   do i=1,Mon%NAct
   xtrace = xtrace + rdm24act(i,i,k,k)
   enddo
enddo
write(lout,'(1x,"Trace of 2-TRDM", F12.6)') xtrace

call get_1rdm_from_2rdm(rdm24act,mon%rdm1(Mon%IRef1,:),Mon%NAct,Mon%INAct,Mon%XELE,1,2)
...
! Gamma0n(p,q,r,s) for p and s inact = - delta_ps gamma0n(r,q)
!
do k=1,Mon%INAct
   do j=1,Mon%NAct
      do i=1,Mon%NAct
...
do k=1,Mon%INAct
   do j=1,Mon%NAct
      do i=1,Mon%NAct
         mat(k,Mon%INAct+i,Mon%INAct+j,k) = -DTNO(Mon%INact+j,Mon%INAct+i)
...
real*8 Function FRDM24(IP,IQ,IR,IS,RDM24Act,Occ,Ind2,NAct,NBasis)
!
! a 4-index version of the FRDM2 subroutine
...
!
 ...
 integer,intent(in) :: Ind2(NBasis)
 double precision,intent(in) :: RDM24Act(NAct,NAct,NAct,NAct)
 double precision,intent(in) :: Occ(NBasis)
 double precision :: RDM2
!
 RDM2=0d0
 If(IP.Eq.IR.And.IQ.Eq.IS.And. (Occ(IP).Eq.1d0.Or.Occ(IQ).Eq.1d0) ) &
  RDM2=RDM2+2d0*Occ(IP)*Occ(IQ)
 If(IP.Eq.IS.And.IQ.Eq.IR.And. (Occ(IP).Eq.1d0.Or.Occ(IQ).Eq.1d0) ) &
  RDM2=RDM2-Occ(IP)*Occ(IQ)
!
! ACTIVE PART
!
 if(Ind2(IP)*Ind2(IQ)*Ind2(IR)*Ind2(IS).Ne.0d0) then
 RDM2 = RDM24Act(Ind2(IP),Ind2(IQ),Ind2(IR),Ind2(IS))
 EndIf
!
 FRDM24=RDM2
!
...
end subroutine read_2rdm_2trdm_trexio_tmp

subroutine read_2rdm_2trdm_trexio(NAO,NBasis,mon)
!
! a) read active 2-RDMs for all states in SA-MO
!    transform them to NOs of a given state,
! b) save 2-RDM for state Mon%IRef1 in RDM2val  
! c) read <IRef2|IRef1> active 2-TRDM in SA-MO
! d) transform from MO--> NO using Mon%CMONO
!    expand to NOccup
...
double precision :: xtrace,xnorm

integer,allocatable          :: idx_buf(:,:)
double precision,allocatable :: val_buf(:)

double precision,allocatable :: DTNO(:,:)
double precision,allocatable :: work(:,:)
double precision,allocatable :: rdm24act(:,:,:,:,:,:)
...
if (rc /= TREXIO_SUCCESS) then
   call trexio_string_of_error(rc, err_msg)
    print *, 'Error reading number of 2-TRDM elements: '//trim(err_msg)
    stop
end if
!print*, 'size_max 2trdm = ',size_max,NBasis

BUFSIZE = 100000

allocate(idx_buf(4,BUFSIZE),val_buf(BUFSIZE))
...
do istate=1,nstates
   ...
   do k=1,NAct
      do i=1,NAct
         xtrace = xtrace + rdm24act(i,i,k,k,istate,istate)
      enddo
   enddo
   xnorm = norm2(rdm24act(:,:,:,:,istate,istate))
   write(lout,'(1x,"2-RDM (act) no. ",I0," has trace = ",F12.6, " and norm =", F12.6)') istate,xtrace,xnorm
   ! active part 
   do l=1,NACT
      do k=1,NACT
         do j=1,NACT
            do i=1,NACT
               mon%rdm24(istate,INACT+i,INACT+j,INACT+k,INACT+l) = rdm24act(i,j,k,l,istate,istate)
               !write(6,'(4i3,f12.8)') INACT+i,INACT+j,INACT+k,INACT+l, rdm24act(i,j,k,l,istate,istate)
            enddo
         enddo
      enddo
   enddo
   ...
   do k=1,NOCCUP
         do i=1,NOCCUP
            if (mon%rdm1(istate,i).Eq.1d0.or.mon%rdm1(istate,k).Eq.1d0) then
               mon%rdm24(istate,i,i,k,k) =  mon%rdm24(istate,i,i,k,k)+2d0*mon%rdm1(istate,i)*mon%rdm1(istate,k)
               mon%rdm24(istate,i,k,k,i) =  mon%rdm24(istate,i,k,k,i)-mon%rdm1(istate,i)*mon%rdm1(istate,k)
            endif
         enddo
   enddo

#if SAPT_INTERFACE_DEBUG > 15
   print*, 'print 2-RDM istate = ',prefix, istate
   do l=1,NOccup
      do k=1,NOccup
         do j=1,NOccup
            do i=1,NOccup
               write(6,'(4i3,f12.8)')i,j,k,l, mon%rdm24(istate,i,j,k,l)
            enddo
         enddo
      enddo
   enddo
   ...
   do k=1,NOccup
      do i=1,NOccup
         xtrace = xtrace + mon%rdm24(istate,i,i,k,k)
      enddo
   enddo
   xnorm = norm2(mon%rdm24(istate,:,:,:,:))
   write(lout, '(1x,"2-RDM full  no. ",I0," has trace = ",F12.6)') istate,xtrace

#if SAPT_INTERFACE_DEBUG > 5
   call get_1rdm_from_2rdm(mon%rdm24(istate,:,:,:,:),mon%rdm1(istate,:),NOccup,INACT,Mon%XELE,1,1)
#endif
...
do j=1,NAct
   do i=1,NAct
      work(i,j) = Mon%CMONO(Mon%IRef1,INAct+i,INAct+j)
   enddo
enddo
call TrRDM24_dgemm(rdm24act(:,:,:,:,jst,ist),work,NAct)

xnorm = norm2(rdm24act(:,:,:,:,jst,ist))
xtrace = 0d0
do k=1,Mon%NAct
   do i=1,Mon%NAct
   xtrace = xtrace + rdm24act(i,i,k,k,jst,ist)
   enddo
enddo
write(lout,'(1x,"2-TRMD <", I2, " |", I2, " >  has trace ", F12.6, "and norm ", F12.6)') jst,ist,xtrace,xnorm

#if SAPT_INTERFACE_DEBUG > 5
   call get_1rdm_from_2rdm(rdm24act(:,:,:,:,jst,ist),mon%rdm1(ist,:),NACT,INACT,Mon%XELE,1,2)
#endif
...
do l=1,NACT
   do k=1,NACT
      do j=1,NACT
         do i=1,NACT
            mon%trdm24(INACT+i,INACT+j,INACT+k,INACT+l) = rdm24act(i,j,k,l,jst,ist)
            !if(abs(rdm24act(i,j,k,l,2,1))>1d-3) then
            !   write(6,'(4i3,f12.8)') INACT+i,INACT+j,INACT+k,INACT+l, rdm24act(i,j,k,l,jst,ist)
            !endif
         enddo
      enddo
   enddo
...
! inactive 2-TRDM part
!
! Gamma0n(p,q,r,s) for all indices inact = 0
! Gamma0n(p,q,r,s) for r and s inact = 2 * delta_rs gamma0n(p,q)
! Gamma0n(p,q,r,s) for p and s inact = - delta_ps gamma0n(r,q)
!
! G0n(pqrs) = <0 | p^dag r^dag s q |n>
!           = delta_rs * <0 | p^dag s^dag s q |n>
!           = 2* delta_rs * g0n(pq)
!
! G0n(pqrs) = <0| p^dag r^dag s q |n>
!           = -delta_ps * <0| r^dag s^dag s q |n>
!           = -delta_ps * g0n(rq)
!
do k=1,INACT
   do j=1,NACT
      do i=1,NACT
         mon%trdm24(INACT+i,INACT+j,k,k) = 2d0*DTNO(INACT+i,INACT+j)
         mon%trdm24(k,k,INACT+i,INACT+j) = mon%trdm24(INACT+i,INACT+j,k,k)
      enddo
   enddo
enddo
do k=1,INACT
   do j=1,NACT
      do i=1,NACT
         mon%trdm24(k,INACT+i,INAct+j,k) = -DTNO(INact+j,INAct+i)
         mon%trdm24(INACT+j,k,k,INAct+i) =  mon%trdm24(k,INAct+i,INAct+j,k)
      enddo
   enddo
enddo

#if SAPT_INTERFACE_DEBUG > 15
write(lout, '(/,"2-TRDM  <", I2, " |", I2, " > CAS for monomer ",A2," norm2 = ",F12.6)') jst,ist,prefix,norm2(mon%trdm24)
do l=1,NOccup
   do k=1,NOccup
      do j=1,NOccup
         do i=1,NOccup
            if(abs(mon%trdm24(i,j,k,l))>1d-3) then
               write(6,'(4i3,f12.8)')i,j,k,l, mon%trdm24(i,j,k,l)
            endif
         enddo
      enddo
   enddo
...
double precision,intent(in) :: P(NACT,NACT,NACT,NACT)
...
case(0)
   do k=1,NACT
      do j=1,NACT
         do i=1,NACT
            DfromP(i,j) = DfromP(i,j) + P(i,k,j,k)
         enddo
      enddo
   enddo
case(1)
   do k=1,NACT
      do j=1,NACT
         do i=1,NACT
            DfromP(i,j) = DfromP(i,j) + P(i,j,k,k)
         enddo
      enddo
   enddo
case default
   stop "Error! wrong call of get_1rdm_from_2rdm!"
end select

if (dtype==1) then
   do i=1,NACT
      val = abs(Occ(i))-abs(DfromP(i,i))
      if (val.gt.1d-4) print*, 'Warning! Occ(i) \= DfromP(i,i) : ', i, val
   enddo
endif
...
end subroutine get_1rdm_from_2rdm
...
#if SAPT_INTERFACE_DEBUG > 15
   print*, 'CMONO (prepare_no_trexio) =',norm2(CMONO)
   do j=1,NBasis
      write(lout,'(*(f12.6))') (CMONO(i,j),i=1,NBasis)
   enddo
   print*, 'CAOMO (prepare_no_trexio) =',norm2(CAOMO)
   do j=1,NBasis
      write(lout,'(*(f12.6))') (CAOMO(i,j),i=1,NBasis)
   enddo
#endif
...
#if SAPT_INTERFACE_DEBUG > 15
   print*, 'CMONO Monomer =',Mon%Monomer,norm2(URe)
   do j=1,NBasis
      write(lout,'(*(f12.6))') (URe(i,j),i=1,NBasis)
   enddo
#endif
...
! END OF CANONICALIZING
 ...
 !if (allocated(Mon%IndInt)) deallocate(Mon%IndInt)
...
subroutine rw_trexio_save_rdm2file(Flags,Mon)
  ...
  do j=1,Mon%NAct
     ...
     do k=1,Mon%NAct
        do l=1,Mon%NAct
           kl = (k-1)*Mon%NAct+l
           if(ij>=kl) then
             write(iunit,'(4i4,f19.12)')  k,i,l,j,2d0*Mon%RDM2val(Mon%INAct+i,Mon%INAct+k,Mon%INAct+j,Mon%INAct+l)
             !write(6,'(4i2,f12.8)')       k,i,l,j,2d0*Mon%RDM2val(Mon%INAct+i,Mon%INAct+k,Mon%INAct+j,Mon%INAct+l)
           endif
        enddo
     enddo
  enddo
...
end subroutine rw_trexio_save_rdm2file
...
Mon%RDM2val = RDM2tru
print*, 'RDM2val =',norm2(Mon%RDM2val)
         ...
         do l=1,Mon%NAct
            ...
            if(ij>=kl) then
              write(iunit,'(4i4,f19.12)') &
               ...
               !write(6,'(4i2,f12.8)') k,i,l,j,2d0*RDM2Act(NAddrRDM(i,j,k,l,Mon%NAct))
            endif
         enddo
      enddo
   enddo
...
!
! dSRS: use CAONO for IRef1
! obtained in read_1rdm_1trdm
!
implicit none

integer,intent(in) :: NAO,NBasis
integer,intent(in) :: idSRS
