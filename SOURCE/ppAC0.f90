...
contains
  ...
  !   double precision, dimension(:,:), allocatable :: MxA1, MxA, MxS
  !   double precision, dimension(:), allocatable :: tempx, tempx2
  !   double precision :: gr, gr2, gr3, sa, tr1, tr2
  !   double precision, dimension(:), allocatable :: Aux22
  !   double precision, dimension(:,:), allocatable :: IMxA1, IMxA2, IMxA3
  !   integer :: z
  ...
  !   allocate(MxA(ePAl%dim, ePAr%dim))
  !   allocate(MxS(ePAl%dim, ePAl%dim))

  !   allocate(IMxA2(ePAr%dim, ePAl%dim))
  !   allocate(IMxA3(ePAl%dim, ePAr%dim))
  !   allocate(Aux22( ePAl%dim *  ePAr%dim))
  ...
  !   print*, 'Czas na wybrany wycinek macierzy A1: '//str(clock_readwall(timer),d=2)
  !   deallocate(MxS)
  ...
  !                  Aux1 = Aux1 + Aux2 * ePAr%Eigvec(j, km)/(ePAl%Eig(np)-ePAr%Eig(km))
  !               end if
  !            end do
  !            IMxA1(np, j) = Aux1
  !         end do
  ...
  !         end do
  !      end do
  !      print*, 'Czas na pierwsze petle: '//str(clock_readwall(timer),d=2)       
  !   end if
  ...
  !         end do
  !      end do
  !      print*, 'Czas na drugie petle: '//str(clock_readwall(timer),d=2)
  ...
  !      print*, 'Czas na trzecie petle: '//str(clock_readwall(timer),d=2)
  ...
  !         end do j_colloops
  !      end do i_rowloops
  !       print*, 'Czas na czwarte petle: '//str(clock_readwall(timer),d=2)

  !   end select
  !   !       !omp end parallel do

  !   print*, ''
  !   print*, 'Czas na timeall  petle: '//str(clock_readwall(timerall),d=2)
  !   print*, ''
                        ...
                        do r = 1, NI+NA
                              do s = 1, NI+NA
                                    ...
                                    if (abs(TwoNO(NAddr3(p, r, q, s))* FRDM2(p, q, r, s, R00, n, Ind2, NA, NBasis)).gt.1.d-5)then
                                          write(*, '(4I5, 3F20.15)')p, q, r, s, TwoNO(NAddr3(p, r, q, s)), FRDM2(p, q, r, s, R00, n, Ind2, NA, NBasis), etot
                                    end if
