...
double precision,intent(inout) :: tmp(A%NDimX,B%NDimX)
...
integer,intent(in) :: nOVB,NBas
double precision,intent(inout) :: tmp1(A%NDimX,B%NDimX),&
                                  sij(A%NDimX,B%NDimX)
   ...
   do irs=1,B%NDimX
      ...
      do i=1,A%NDimX
         tmp1(i,irs) = tmp1(i,irs) + &
                      fact * &
                      (A%EigY(ipq+(i-1)*A%NDimX)-A%EigX(ipq+(i-1)*A%NDimX))
      enddo
...
do j=1,B%NDimX
   do i=1,A%NDimX
      do irs=1,B%NDimX
         ...
         is = B%IndN(2,irs)
         sij(i,j) = sij(i,j) + &
                    (B%EigY(irs+(j-1)*B%NDimX)-B%EigX(irs+(j-1)*B%NDimX))*tmp1(i,irs)
      enddo
   enddo  
 enddo
...
double precision,intent(inout) :: tmp1(A%NDimX,B%NDimX),&
                                  tmp2(A%NDimX,B%NDimX),&
                                  tmp3(A%NDimX,B%NDimX)
