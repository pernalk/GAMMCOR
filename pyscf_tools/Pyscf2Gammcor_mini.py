...
class _Dump:
    ...
    def text(self, legacy, build):
        """Write the ASCII-only rdm2.dat payload."""
        with open(legacy, 'w') as f:
            f.write(build())
    ...
    def provenance(self, method):
        ...
        with h5py.File(self.filename, 'a') as f:
            ...
            f.attrs['VERSION'] = 1
            f.attrs['GENERATOR'] = 'Pyscf2Gammcor_mini.py'
...
def get_rohf_for_gammcor(mol, myhf, dump_hdf5 = True):
    ...
    get_rohf_for_gammcor in Pyscf2Gammcor.py; dump_hdf5 only changes where
    they land.
    ...
    # ===================================================================
    # Krok 2: Zbuduj 2-RDM. Konwencja dokladnie jak w Pyscf2Gammcor.py:
